...
class FlexiPosEmbed(nn.Module):
    ...
    def forward(self, x, patch_size=None, strides=None, token_position=None):
        ...
        if patch_size is None and strides is None:
            print(f"Shape of x before position embedding: {x.shape}")  # Should be (B, N_patches, D)
            print(f"Shape of self.pos_embed: {self.pos_embed.shape}")  # Should match (1, N_patches+1, D)
            if self.pos_embed.shape[1] > x.shape[1]:  # If pos_embed is too big, slice it
                self.pos_embed = torch.nn.Parameter(self.pos_embed[:, :x.shape[1], :], requires_grad=False)
            elif self.pos_embed.shape[1] < x.shape[1]:  # If pos_embed is too small, interpolate
                self.pos_embed = torch.nn.functional.interpolate(
                    self.pos_embed, size=(x.shape[1], self.pos_embed.shape[2]), mode="linear"
                )
            x = x + self.pos_embed

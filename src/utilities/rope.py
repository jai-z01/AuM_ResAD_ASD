...
class VisionRotaryEmbedding(nn.Module):
    def __init__(
        ...
        pt_seq_len = to_2tuple(pt_seq_len)
        ft_seq_len = to_2tuple(ft_seq_len)
    ...
    def forward(self, t, num_cls_tokens=1):
        t_spatial = t[:, num_cls_tokens:, :]
        t_spatial = t_spatial * self.freqs_cos + rotate_half(t_spatial) * self.freqs_sin
        return torch.cat((t[:, :num_cls_tokens, :], t_spatial), dim=1)

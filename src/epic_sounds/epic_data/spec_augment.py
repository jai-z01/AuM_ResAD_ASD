...
'''
Code Imported from: https://github.com/zcaceres/spec_augment
'''
...
#         matrix_a += regularization_weight * batch_identity_matrix
    ...
    try:
        ...
        X = torch.matmul(torch.linalg.pinv(lhs), rhs)
    w = X[:, :n, :]
    v = X[:, n:, :]
    return w, v
...
def combined_transforms(spec,cfg):
    t_mask = cfg.T_MASK
    f_mask = cfg.F_MASK
    warp = cfg.T_WARP
    return time_mask(freq_mask(time_warp(spec,W=warp), num_masks=2,F=f_mask), num_masks=2, T=t_mask)


def combined_transforms_flexi(spec,cfg):
    spec_len = spec.shape[2]
    t_mask = cfg.T_MASK * spec_len // 1024
    f_mask = cfg.F_MASK
    warp = cfg.T_WARP * spec_len // 1024
    # import ipdb; ipdb.set_trace()
    if warp == 0:
        return time_mask(freq_mask(spec, num_masks=2,F=f_mask), num_masks=2, T=t_mask)
    else:
        return time_mask(freq_mask(time_warp(spec,W=warp), num_masks=2,F=f_mask), num_masks=2, T=t_mask)

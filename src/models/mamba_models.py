...
class AudioMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, patch_size=None, strides=None):
        x = x.unsqueeze(1)  
        x = x.transpose(2, 3)
        x = x.squeeze(3)
        #print(f"Final x.shape before patch_embed = {x.shape}")

        # Pass to patch embedding
        B, C, F, T = x.shape
        ...
        if not self.if_bidirectional:
            ...
            for i in range(len(self.layers) // 2):
                ...
                hidden_states_f, residual_f = self.layers[i * 2](hidden_states, residual, inference_params=inference_params)
                hidden_states_b, residual_b = self.layers[i * 2 + 1](hidden_states.flip([1]), None if residual is None else residual.flip([1]), inference_params=inference_params)
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, patch_size=None, strides=None): # NOTE: For now, these are all being used as default. Later, these could be set through the args param
        x = self.forward_features(x, inference_params, patch_size=patch_size, strides=strides)
        #x = x.unsqueeze(1)
        #x = F.interpolate(x, size=(256, 768), mode="bilinear", align_corners=False)
        #x = x.squeeze(1)

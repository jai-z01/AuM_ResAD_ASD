...
from datasets.dcase import DCASE2024
...
from classes import MVTEC_TO_MPDD, MVTEC_TO_MVTECLOCO, MVTEC_TO_BRATS, DCASE_Dev_to_Eval
...
def main(args):
    ...
    DCASE_CLASSES = ['bearing', 'fan', 'gearbox', 'slider', 'ToyCar', 'ToyTrain', 'valve']

    if CLASSES['seen'][0] in DCASE_CLASSES:  # Using DCASE2024 dataset
        train_dataset1 = DCASE2024(
            root=args.train_dataset_dir,
            machine_types=CLASSES['seen'],
            train=True,
            normalize="logmel",
            img_size=224, crp_size=224
        )

        train_loader1 = DataLoader(
            train_dataset1, batch_size=args.batch_size, shuffle=True, num_workers=8, drop_last=True
        )
        #for images, labels, masks, class_names in train_loader1:
        #    print(f"Batch image shape: {images.shape}")  # Expect [batch_size, 3, H, W]
        #    break

    else:
        raise ValueError("Unknown dataset type for the given class names.")
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            #print(f"Before conversion, shape: {images.shape}")

            # Expand grayscale spectrograms to 3 channels
            if images.shape[1] == 1:  # Ensure it's a single-channel input
                images = images.repeat(1, 3, 1, 1)

            # Debugging: Check shape after modification
            #print(f"After conversion, shape: {images.shape}")

...
class MVTEC3D(Dataset):
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        #assert classname in MVTEC3D_CLASS_NAMES, 'class_name: {}, should be in {}'.format(classname, MVTEC3D_CLASS_NAMES)
        self.root = root
        ...
        if self.class_name is None:  # load all classes
            ...
            self.image_paths, self.labels, self.mask_paths, self.img_types = self._load_data(class_name)
        # set transforms
        ...
        self.target_transform = T.Compose([
            T.Resize(kwargs.get('img_size'), T.InterpolationMode.NEAREST),
    ...
    def __getitem__(self, idx):
        ...
        return image, label, mask, img_type
    ...
    def _load_data(self, class_name):
        ...
        gt_dir = os.path.join(self.root, class_name, 'test')
    ...
    def _load_all_data(self):
        ...
        for class_name in self.CLASS_NAMES:
            image_paths, labels, mask_paths, types = self._load_data(class_name)
...
def get_normal_image_paths_mvtec3d(root, class_name):
    phase = 'train'
    image_paths = []

    img_dir = os.path.join(root, class_name, phase)
